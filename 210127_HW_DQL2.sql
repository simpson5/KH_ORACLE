...
from tb_student
 where extract(year from entrance_date) -  extract(year from to_date(substr(student_ssn, 1, 2), 'rr')) + 1 > 19;
 
-- 6. 2020 년 크리스마스는 무슨 요일인가?
select to_char(to_date(0201225) , 'day')
from dual;
...
select decode(grouping(substr(term_no,1,4)), 0, substr(term_no,1,4), 1, ' ') 년도,
            decode(grouping(substr(term_no, 5)), 0, substr(term_no, 5), 1, ' ') 학기,
           round( avg(point), 1) "년도 별 평점",
           sum(point) 누적평점
from tb_grade
where student_no = 'A112113'
