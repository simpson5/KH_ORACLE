...
select emp_name 이름,
           '19' || substr(emp_no, 1, 2) 년생,
            nvl(bonus, 0) 보너스
from employee
where substr(emp_no,1,6) between '600101' and '691231';
...
*/
-- 1. 문자형인 주민번호에서 생년월일을 추출한뒤 날짜형으로 형변환한다.
-- 2. 00년생 이전과 이후로 나눈다.
select emp_name 직원명,
            dept_code 부서코드,
            to_char(to_date(
            case
                when substr(emp_no,1,2) > 20 then '19'||substr(emp_no,1,6)
                when substr(emp_no,1,2) < 20 then '20'||substr(emp_no,1,6)
            end,
...
select sum(decode(extract(year from hire_date), '1998', '1' , '0')) "1998년",
           ...
           count(*) 전체직원수
from employee;
-- 그룹 함수 별칭에서 숫자와 한글을 같이 쓰면 오류가 나는건가?
/*
